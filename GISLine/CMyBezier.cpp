...
// CMyBezier 成员函数
double CMyBezier::mypow(double r, int k) {
	if (k < 0)return 0;
	return pow(r, k);
}

Point CMyBezier::dev(vector<Point>& ct, double t) {
	Point ans = (ct[1] - ct[0]) * (3 * mypow(1 - t, 2)) + (ct[2] - ct[1]) * (6 * t * (1 - t)) + (ct[3] - ct[2]) * (3 * t * t);
	return ans;
}

Point CMyBezier::dev2(vector<Point>& ct, double t) {
	Point ans;
	ans = (ct[0] + ct[2] - ct[1] * 2) * (6 * (1 - t)) + (ct[1] + ct[3] - ct[2] * 2) * (6 * t);
	return ans;
}

Point CMyBezier::at(vector<Point>ct, double t) { 
	Point ans = ct[0] * mypow(1 - t, 3) + (ct[1] * (1 - t) + ct[2] * t) * (1 - t) * t * 3  + ct[3] * mypow(t, 3);
	return ans;
}


void CMyBezier::computeMaxError(vector<Point>& pts, const int& first, const int& last, vector<Point>& ctrls, vector<double> params, double& maxDis, int& splitPoint)
{
	double md = 0.0;
	int sp = (first + last) / 2;
	int sz = last - first + 1;
...
vector<Point> CMyBezier::generateBezier(vector<Point>& pts, const int& first, const int last, vector<double> parameters, Point left_tag, Point right_tag)
{
	vector<Point> ans(4);
...
vector<double> CMyBezier::chordLengthParameterize(vector<Point>& pts, const int& first, const int& last) {
	vector<double> us = { 0 };
...
double CMyBezier::newtonRaphsonRootFind(vector<Point>& ctrls, const Point& p, const double& u)
{
...
vector<double> CMyBezier::reparameterize(vector<Point>& ctrls, vector<Point>& pts, vector<double> parameters, const int& first, const int& last) {
	vector<double> ans;
...
vector<Point> CMyBezier::fit_cubic(vector<Point>& pts, int first, int last, Point left_tangent, Point right_tangent, const double& maxErr) {
	vector<Point> ans;
...
vector<Point> CMyBezier::fit_curve(vector<Point>& pts, const double& maxE) {
	int sz = pts.size();
	vector<Point> ans;
